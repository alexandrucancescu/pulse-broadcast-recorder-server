...
class RtpReceiver extends EventEmitter {
	...
	private handleMessage(data: Buffer, remoteInfo: RemoteInfo) {
		...
		setImmediate(() => this.logRtpMessage(remoteInfo, packet))
	...
	private logRtpMessage(remoteInfo: RemoteInfo, packet: RtpPacket) {
		...
		if (
			remoteInfo.address !== this.sourceAddress ||
			Date.now() - this.lastPacketTime > 500
		) {
			log.info(
				`New RTP source connected from ${remoteInfo.address}`
			)

			this.sourceAddress = remoteInfo.address

			log.info(
				`New RTP source payload type = ${packet.payloadType}`
			)
		}
	}
